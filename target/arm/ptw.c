                             ...
                             ARMMMUFaultInfo *fi)
{
    ...
    if (unlikely(!ptw->out_rw)) {
        ...
        env->tlb_fi = fi;
        flags = probe_access_flags(env, ptw->out_virt, sizeof(uint64_t), MMU_DATA_STORE,
...
#endif
    if (!(result->f.prot & (1 << base_access_type))) {
        fi->type = ARMFault_Permission;
        goto do_fault;
    }
                                   ...
                                   ARMMMUFaultInfo *fi)
{
    ...
    result->f.prot = ((result->f.prot ^ PAGE_C_BITS) &
                      (result->f.prot ^ PAGE_C_BITS)) ^ PAGE_C_BITS;
