...
{
    uint64_t ret =
        cpu_load_helper_no_log(env, addr, oi, retaddr, full_load);
    log_instr_load_int(env, retaddr, ret, get_memop(oi));
    return ret;
}
...
static inline void QEMU_ALWAYS_INLINE
cpu_store_helper_no_log(CPUArchState *env, target_ulong addr,
...
static inline void QEMU_ALWAYS_INLINE
cpu_store_helper(CPUArchState *env, target_ulong addr,
                 uint64_t val, MemOpIdx oi, uintptr_t ra,
                 FullStoreHelper *full_store)
{
    cpu_store_helper_no_log(env, addr, val, oi, ra, full_store);
    log_instr_store_int(env, addr, val, get_memop(oi));
}
