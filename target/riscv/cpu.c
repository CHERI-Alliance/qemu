...
static void riscv_cpu_dump_state(CPUState *cs, FILE *f, int flags)
{
    ...
    qemu_fprintf(f, " %s " TARGET_FMT_lx "\n", "mscratch", GET_SPECIAL_REG_ARCH(env, mscratch, MScratchC));
#endif
...
void rvfi_dii_communicate(CPUState* cs, CPURISCVState* env, bool was_trap) {
    ...
    while (true) {
        ...
        case 1: {
            ...
            env->rvfi_dii_trace.INST.rvfi_ixl = env->misa_mxl;
            resume_all_vcpus();
            cpu_resume(cs);
            ...
            cs->exception_index = RISCV_EXCP_NONE; // EXCP_INTERRUPT;
            cs->cflags_next_tb = (curr_cflags(cs) & ~CF_USE_ICOUNT) | 1;
...
static Property riscv_cpu_properties[] = {
    ...
    DEFINE_PROP_BOOL("cheri_v090", RISCVCPU, cfg.cheri_v090, true),
