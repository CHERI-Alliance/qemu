                       ...
                       target_ulong vaddr, CPUTLBEntryFull *full)
{
    ...
    uintptr_t tagmem = (uintptr_t)cheri_tagmem_for_addr(
        env, vaddr, section->mr->ram_block, xlat, sz, &prot, tag_setting);
...
#ifdef TARGET_CHERI
        if (cap_write && ((env_tlb(env)->d[mmu_idx].vfulltlb[vidx].tagmem_write &
                      ...
                      uintptr_t retaddr)
{
    ...
    if (access_type == MMU_DATA_CAP_STORE &&
        ((env_tlb(env)
              ->d[mmu_idx]
              .fulltlb[tlb_index(env, mmu_idx, addr)]
              .tagmem_write &
                                ...
                                uintptr_t retaddr)
{
    ...
    oi = make_memop_idx(MO_TEUQ, cpu_mmu_index(env, false));
